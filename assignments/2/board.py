...
def print_boards_with_max(boards, max_per_line=-1):
    if(max_per_line == -1):
        max_per_line = len(boards)

    for k in range(len(boards)//max_per_line + 1):
        print_boards(boards[max_per_line*k : max_per_line*(k+1)])
